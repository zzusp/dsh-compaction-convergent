...
import LlmRuntime, { createUserMessage, ToolCallId as CallId, CONTEXT_WINDOW_EXCEEDED_CODE, createToolResultMessage, LlmAdapter , createMessage } from '@deepseek-ai/dsh-llm'
import type {
  ContentBlock,
  GenerateOptions,
  LlmFailure,
...
describe('pressure measurement and retention', () => {
  ...
  it('declines when envelope pressure is high but the surface has no compactable range', async () => {
    ...
    await expect(compactIfNeeded(compact, retained))
  ...
  it('expands the selected range only after a typed non-shrinking failure', async () => {
    const compact = service(compactConfig)
    const session = conversation(4)
    const stop = new Error('stop after observing expanded range')
    const compactRegion = vi.spyOn(compact, 'compactRegion')
      .mockRejectedValueOnce(new SummaryNotSmallerError(200, 100))
    ...
    const firstEnd = compactRegion.mock.calls[0]![1]
    const expandedEnd = compactRegion.mock.calls[1]![1]
    expect(expandedEnd).toBeGreaterThan(firstEnd)
  })

  it('commits an expanded real transaction after the initial checkpoint does not shrink', async () => {
    const compact = service(compactConfig)
    compact.mutateDuringSummary = () => {
      compact.summary = compact.calls.length === 1
        ? Array.from({ length: 100 }, (_, index) => ({
            type: 'text',
            text: `verbose ${index}`,
          }))
        : [{ type: 'text', text: 'converged checkpoint' }]
    }
    const session = conversation(4)
    const generation = session.surface.replaceGeneration
    ...
    expect(compact.calls[1]!.input.messages.length)
      .toBeGreaterThan(compact.calls[0]!.input.messages.length)
  ...
  it('tries each distinct retained and maximal range only once', async () => {
    ...
    const compactRegion = vi.spyOn(compact, 'compactRegion')
      .mockRejectedValue(new SummaryNotSmallerError(200, 100))
  ...
  it('suppresses a failed range across pressure checks until the surface generation advances', async () => {
    ...
    session.append('user/message', createUserMessage({
      content: [{ type: 'text', text: 'surface generation advance' }],
      source: { kind: 'plugin', plugin: 'test' },
    }), {
  ...
  it('terminates when a claimed successful chunk makes no replacement-generation progress', async () => {
    ...
    const fake: CompactionResult = {
      ...
      startSeq: SessionSeq(0),
      summarySeq: SessionSeq(0),
      endSeq: SessionSeq(0),
  ...
  it('repairs closed-step orphan calls only inside maximal summarization input', async () => {
    ...
    expect(session.snapshotEvents().some(event => event.type === 'tool/result')).toBe(false)
  })
...
describe('compaction region transaction', () => {
  ...
  it('rejects a non-shrinking framed summary under the conversation meter', async () => {
    ...
    const failure = compact.compactRegion(
      nodes[0]!,
      nodes[2]!,
      agent(session, MODEL),
    )
    await expect(failure).rejects.toBeInstanceOf(SummaryNotSmallerError)
    await expect(failure).rejects.toThrow(/summary is not smaller/)
    expect(session.snapshotEvents().some(event => event.type === 'compaction/summary')).toBe(false)
  })
...
describe('summary input capacity convergence', () => {
  ...
  it('chunks large-envelope pressure until the measured request falls below threshold', async () => {
    ...
    expect(session.snapshotEvents().filter(event => event.type === 'compaction/summary'))
  ...
  it('finishes a large surface in one job beyond the legacy successful-chunk cap', async () => {
    ...
    const records = session.snapshotEvents().flatMap(event => {
    ...
    const lifecycles = session.snapshotEvents().filter(event => (
  ...
  it('restores an unfinished job and learned capacity from durable compaction events', async () => {
    ...
    void installTokenMeter(firstCtx)
    ...
    const firstRecords = original.snapshotEvents().flatMap(event => {
      const record = convergenceRecord(event)
      return record === undefined ? [] : [record]
    })
    ...
    const resumed = Session.create(SessionId('capacity-job-resumed'), [...original.snapshotEvents()])
    const secondCtx = new Context()
    void new LlmRuntime(secondCtx)
    void installTokenMeter(secondCtx)
    ...
    const allRecords = resumed.snapshotEvents().flatMap(event => {
      const record = convergenceRecord(event)
      return record === undefined ? [] : [record]
    })
  ...
  it('shrinks after provider context overflow and submits each range at most once', async () => {
    ...
    expect(session.snapshotEvents().filter(event => event.type === 'compaction/summary')).toHaveLength(1)
    expect(session.snapshotEvents().filter(event => event.type === 'compaction/start'))
      .toHaveLength(adapter.requests.length)
    expect(session.snapshotEvents().filter(event => event.type === 'compaction/end'))
      .toHaveLength(adapter.requests.length)
  })
...
describe('automatic listener and loader composition', () => {
  ...
  it('recovers an overflow for an unlisted routed model', async () => {
    ...
    expect(await recover(ctx, agent(session, MODEL), overflow('unlisted-model overflow')))
      .toBe(true)
    const record = session.snapshotEvents()
