...
- **Convergence** — one pressure or context-overflow trigger creates a stable logical job. After every successful chunk it remeasures and, while the complete request remains at or above threshold, selects another balanced head span; `compactionRetries` no longer truncates successful replacements. A non-shrinking summary still expands through `retainTokens`, `floor(retainTokens / 2)`, and `0`, and duplicate `start:end` ranges never call the summarizer twice. Continuing requires both replacement-generation progress and a strict token decrease, otherwise the job fails finitely.
- **Capacity learning** — the default summarizer keys capacity by provider, model, context window, `maxTokens`, system/tools, compaction instruction, fixed envelope, and TokenMeter strategy. Provider rejection tightens the smallest rejected replay, while success raises the largest accepted replay; later generations select a balanced prefix below the accepted bound minus a safety margin instead of probing the same large range. A provider-reported actual/maximum token gap is preferred, with conservative halving as fallback. Any key input change naturally invalidates the old profile.
- **Closed-step orphan calls** — normal pairing boundaries still reject unanswered tool calls. Only when every unanswered call belongs to an old step with a durable `step/end`, and ordinary ranges cannot shrink, may the final maximal range cover the whole surface. The summarization input inserts an error result immediately after each such call without writing repairs into the source Session log. Calls in a currently open step never use this path.
- **Recovery boundary** — the provider can compact only the current Session surface. A `session/end-seed` appended while DSH restores historical state creates a new recovery surface, so online automatic compaction cannot reselect pathological history before that boundary. Such a Session needs a one-time real-model repair before Web recovery; the deterministic placeholder test script is not suitable for production semantic migration.
- **Summarization** — a direct `llm/stream` call uses the configured provider/model pair and cap, falling back to the latest logged request target and then the agent target, without running the loop-only `agent/request` extension point. Before dispatch, the default summarizer prices the complete request — system, tools, selected messages, trailing instruction, and the `maxTokens` output reserve — against that model's advertised context window. The call replays the conversation's own system prompt, tools, and shadowed-region messages verbatim, including image references, and appends the compaction instruction as the final user message, so it reuses the provider's warm prefix cache instead of invalidating it. The selected adapter must resolve or explicitly reject those images. It sets `GenerateOptions.purpose` to `compaction`, which adapters may forward as request attribution (the DeepSeek adapter sends `x-deepseek-harness-compact: 1`) without touching the model-visible body. Only returned text enters the checkpoint, excluding reasoning and tool calls that would leak private reasoning or create an orphaned call; image output fails with `UNSUPPORTED_CONTENT` rather than disappearing.
...
- **Lifecycle** — all entry points share one bracket-first region transaction. It validates the range and live lock, appends `compaction/start` synchronously, prepares and awaits the summary, revalidates, appends `compaction/summary` plus the replacement, and makes exactly one closing attempt. Automatic transactions add a `convergence` diagnostic to those existing events, durably recording jobId, chunk/attempt, request and Surface tokens, range, capacity bounds, and continue/completed state; a rebuilt engine can resume an unfinished job and compatible capacity from the log. Automatic and explicit-region calls require a numeric open-turn owner and whole-surface stability; the serial `agent/pre-step` listener checks pressure before request derivation, while canonical provider overflow enters through `agent/request-error` and authorizes retry only after durable surface progress. `compactNow()` reserves idle admission, uses `turn: null`, accepts append-only context outside its selected span, flushes every closed attempt, and releases admission in `finally`.
- **Overflow recovery** — provider-confirmed overflow needs no capacity metadata to qualify. Its first chunk bypasses normal pressure and retention, prunes, then targets a maximal balanced head reduction while leaving the newest indivisible unit; when route capacity is known and pressure remains high, later chunks restore the recent-tail policy and continue in the same job. When the summary model advertises capacity, an oversized target is reduced to the largest balanced prefix whose complete summary envelope fits before any model call. A canonical summary-side `CONTEXT_WINDOW_EXCEEDED` rejection keeps reducing toward a smaller, previously untried balanced prefix; rejected `start:end` ranges remain suppressed for the current generation while learned capacity survives generation changes. If the oldest indivisible unit still cannot be submitted, `OversizedSurfaceUnitError` reports its seq range and token/envelope budget instead of looping. Retry is authorized whenever `surface.replaceGeneration` advances, including when pruning lands before later summary work throws. No replacement, an exhausted target-specific cap, cancellation, or an unknown/noncanonical error preserves the original provider failure.
...
The protected `summarize()` method is the sole subclass hook. A template- or remote-summarizer subclass can override it while pressure, retention, cited source events, shrink validation, and shadowed-token accounting stay on `ctx.tokenMeter`. The hook returns the safe summary plus the complete provider output, call envelope, and usage when available (`{ summary, rawOutput?, llmStreamCall?, provider, model, maxTokens?, usage?, summaryEnvelope? }`); `llmStreamCall: true` means producing that result consumed exactly one call through this context's `ctx.llm.stream()` and requires complete `rawOutput`, while unmarked `rawOutput` does not identify the call path. The default summarizer supplies `summaryEnvelope` for capacity learning; a custom summarizer that omits it still converges continuously but gets no guessed capacity profile. The transaction preserves those fields on `compaction/summary`.
...
| `compactionRetries` | no (default `1`) | Retained for configuration compatibility; it no longer caps successful chunks within one automatic job. |
...
import { BasicCompactionEngine } from '@zzusp/dsh-compaction-convergent'
import SessionStore from '@deepseek-ai/dsh-session'
import TokenMeter from '@deepseek-ai/dsh-token-meter'

export const name = 'compaction-convergent'
...
The summarization model receives the conversation replayed from the selected surface — the same system prompt and tool schemas, plus the original messages except for synthetic error results inserted after closed-step orphan calls — followed by one final user message: the compaction instruction below. These repairs exist only in the auxiliary request and are not appended to the Session log. The conversation model never sees this private request or its reasoning; only returned text is stored.
...
This is a separate model call: the replayed conversation prefix plus the fixed instruction as input, with `maxTokens`-capped output. A preflight-rejected range makes no model call; a provider-rejected range is paid at most once per surface generation before the next smaller balanced prefix is tried.
