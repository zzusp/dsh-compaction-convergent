...
The **convergent compaction backend** is a source-compatible replacement for `@deepseek-ai/dsh-compaction-basic`. It keeps the official `BasicCompactionEngine` transaction and service contracts while ensuring automatic pressure compaction does not repeatedly pay for the same non-shrinking range.
...
- **Convergence** — reject a summary that does not shrink its source, then retry that successful-replacement slot with retained-tail budgets `retainTokens`, `floor(retainTokens / 2)`, and `0`. Every budget change remeasures and reselects the surface; duplicate `start:end` ranges do not call the summarizer twice. Only `SummaryNotSmallerError` expands the range. After a successful replacement, the official `compactionRetries` behavior remains unchanged.
...
- **Overflow recovery** — provider-confirmed overflow needs no capacity metadata to qualify: it bypasses normal pressure and retention, prunes, then targets one maximal balanced head reduction while leaving the newest indivisible unit. When the summary model advertises capacity, an oversized target is reduced to the largest balanced prefix whose complete summary envelope fits before any model call. A canonical summary-side `CONTEXT_WINDOW_EXCEEDED` rejection halves toward a smaller, previously untried balanced prefix; rejected `start:end` ranges remain suppressed for the current `surface.replaceGeneration`. If the oldest indivisible unit still cannot be submitted, `OversizedSurfaceUnitError` reports its seq range and token/envelope budget instead of looping. Retry is authorized whenever `surface.replaceGeneration` advances, including when pruning lands before later summary work throws. No replacement, an exhausted target-specific cap, cancellation, or an unknown/noncanonical error preserves the original provider failure.
