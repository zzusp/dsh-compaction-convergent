...
- **历史恢复边界需要显式 repair**：Web 恢复已持久化 Session 后会追加 `session/end-seed`；病理历史的首次处理应在 Resident 恢复前使用 `@zzusp/dsh-compaction-convergent/repair` 对副本执行真实模型分层摘要。该入口校验 SHA-256/Session ID，只创建新输出，不覆盖输入。
